/*
  ...
  2. Create fresh schema for task management system
  3. Set up proper RLS policies
  4. Create helper functions for user management
  
  ## New Tables
  - `companies` - Company information
  - `users` - User profiles with roles (admin/staff)
  - `teams` - Teams within companies
  - `user_teams` - Team memberships with leader flags
  - `tags` - Tasks with full assignment and tracking
  - `tag_comments` - Comments and status updates
  - `password_resets` - Admin-managed password resets
...
DROP TABLE IF EXISTS tag_comments CASCADE;
...
DROP FUNCTION IF EXISTS create_user_with_password(text, text, text, text, uuid) CASCADE;
...
RETURNS uuid AS $$
BEGIN
  RETURN (SELECT company_id FROM users WHERE id = p_user_id);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  IF NOT is_company_admin(auth.uid(), p_user_company_id) THEN
...
BEGIN
  -- Check if the requesting user is an admin of the company
  IF NOT is_company_admin(auth.uid(), p_requesting_company_id) THEN
  ...
  VALUES (p_target_user_id, p_requesting_company_id, auth.uid())
