-- Complete database migration with proper dependency handling
-- This migration sets up the full company-based task management system

-- First, drop all existing policies that depend on functions
...
-- Drop all password reset request policies
...
-- Enable RLS on password_reset_requests if not already enabled
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create new policies for user_invitations (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    ...
    AND policyname = 'Company members can view invitations'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'user_invitations' 
    AND policyname = 'Company leaders can create invitations'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'user_invitations' 
    AND policyname = 'Company leaders can update invitations'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'user_invitations' 
    AND policyname = 'Company leaders can delete invitations'
  ) THEN
...
-- Create policies for password_reset_requests (only if they don't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'password_reset_requests' 
    AND policyname = 'Company admins can create password reset requests'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'password_reset_requests' 
    AND policyname = 'Company admins can view password reset requests'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'password_reset_requests' 
    AND policyname = 'Users can view their own password reset requests'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'password_reset_requests' 
    AND policyname = 'Company admins can update password reset requests'
  ) THEN
